...
namespace MagicLeap.OpenXR.Features.PixelSensors
{
    public partial class MagicLeapPixelSensorFeature
    {
        ...
        /// </summary>
        /// <param name="path">The XrPath string of the sensor. /pixelsensor/world/left for example</param>
        /// <param name="sensorId">The PixelSensorId associated with the sensor</param>
        /// <returns>True if a sensor was found, false otherwise</returns>
        public bool GetPixelSensorFromXrPath(string path, out PixelSensorId sensorId)
        {
            var xrPath = StringToPath(path);
            if(!pathsFetched)
            {
                GetAvailableSensorsInternal();
            }

            sensorId = supportedSensors.FirstOrDefault(sensor => sensor.XrPath == xrPath);
            return sensorId != default;
        }
        ...
        /// <para>A configured value is the value that is applied to the sensor by calling <see cref="ApplySensorConfig(MagicLeap.OpenXR.Features.PixelSensors.MagicLeapPixelSensorFeature.PixelSensorId,MagicLeap.OpenXR.Features.PixelSensors.MagicLeapPixelSensorFeature.PixelSensorConfigData)"/></para>
        /// </summary>
        ...
        public bool GetSensorData(PixelSensorId sensorType, uint streamIndex, out PixelSensorFrame frame, out PixelSensorMetaData[] metaData, Allocator allocator, long timeOut = 10, bool shouldFlipTexture = true)
        {
            ...
            unsafe
            {
                ...
                frame.CreateFromNativeFrame(sensorFrameInternal, allocator, shouldFlipTexture, sensor.StreamBuffers[streamIndex].LastCaptureTime);
                return true;
            }
        }
