...
namespace UnityEngine.XR.OpenXR.Features.MagicLeapSupport
{
    ...
    public partial class MagicLeapPixelSensorFeature
    {
        internal partial class PixelSensor
        {
            ...
            private ulong sensorSpace;

            private MagicLeapPixelSensorFeature PixelSensorFeature { get; }
            private PixelSensorNativeFunctions NativeFunctions { get; }
            ...
            public bool CreatePixelSensor()
            {
                unsafe
                {
                    ...
                    var xrResult = NativeFunctions.XrCreatePixelSensor(PixelSensorFeature.XRSession, ref createInfo, out var sensorHandle);
            ...
            public Pose GetSensorPose(Pose offset)
            {
                unsafe
                {
                    ...
                    if (sensorSpace == 0)
                    {
                        ...
                        var xrResult = NativeFunctions.XrCreatePixelSensorSpace(PixelSensorFeature.XRSession, ref createSpaceInfo, out sensorSpace);
                    ...
                    var pose = spaceInfoFunctions.GetUnityPose(sensorSpace, PixelSensorFeature.XRSpace, PixelSensorFeature.NextPredictedDisplayTime);
                    return pose;
                }
            }
            ...
            public unsafe bool GetSensorData(uint streamIndex, out XrPixelSensorFrame* sensorFrame, out PixelSensorMetaData[] metaData, Allocator allocator, long timeout, bool shouldFlipTexture)
            {
                ...
                for (var i = 0; i < requestedMetaData.Count; i++)
                {
                    metaData[i] = requestedMetaData[i] switch
                    {
                        ...
                        PixelSensorMetaDataType.DepthConfidenceBuffer => metaDataContainer.DepthConfidenceBuffer.GetMetaData(allocator, shouldFlipTexture),
                        PixelSensorMetaDataType.DepthFlagBuffer => metaDataContainer.DepthFlagBuffer.GetMetaData(allocator, shouldFlipTexture),
            ...
            internal (PixelSensorCapabilityRangeType, PixelSensorCapabilityDataType) GetRangeAndDataType(PixelSensorCapabilityType capabilityType, uint streamIndex)
            {
            ...
            public bool QuerySensorCapability(PixelSensorCapabilityType capabilityType, uint streamIndex, out PixelSensorCapabilityRange capabilityRange)
            {
                ...
                unsafe
                {
                    ...
                    {
                        if (IsRangeTypeDiscrete(capabilityRangeType))
                        {
            ...
            internal void PollSensorStatus()
            {
            ...
            internal bool EnumeratePixelSensorMetadata(uint streamIndex, out PixelSensorMetaDataType[] metaDataTypes)
            {
            ...
            internal bool IsMetaDataTypeSupported(PixelSensorMetaDataType metaDataType, uint streamIndex)
            {
            ...
            internal class PixelSensorStreamStatus
            {
